...
| **arithmetic-overflow/÷0-on-arg** | `/`, `%`, or `abs()`/negate on a Python-controlled value a validator fails to reject (÷0, or `i32::MIN.abs()`) | **CRYPTOGRAPHY-0001** (`PKCS7(0)`/`ANSIX923(0)` ÷0), **PYDANTIC-0001** (`int_schema(multiple_of=0)` ÷0), **PYDANTIC-0004** (`TzInfo(-1e18)` → `i32::MIN.abs()` negate-overflow) | a validator that rejects the degenerate value up front / uses `unsigned_abs`; the same check already raises the intended error for every non-degenerate value | panic-safety-checker |
